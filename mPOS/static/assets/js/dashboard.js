var SampleColors = ["#A7001A", "#014687", "#EE8E06", "#00FFFF", "#FF00FF", "#C0C0C0", "#00CC00", "#3333CC", "#0d5a16", "#030000", "#ef0d5e", "#591f34", "#02d1ff", "#bc00ff", "#2100f2", "#2100f2", "#2100f2", "#99ac14", "#ff0000", "#058e29", "#FF0099"];
  ...
  if (graphs) {
    ...
    Object.values(graphs).forEach(graph => {
      if (graph && graph.key) {
        ...
        if(graph.data&&graph.labels){
          graph.data.forEach((row,index)=>{
            data.push(row[graph.labels['value']]);
            labels.push(row[graph.labels['label']]);
            colors.push(returnColorCode(index));
          })
        }
        var chartData = {
          datasets: [{
          ...
          var chartCanvas = $("#" + graph.key).get(0).getContext("2d");
          ...
          document.getElementById('chart-legends' + graph.key).innerHTML = pieChart.generateLegend();
          }
      }
    });
  }
